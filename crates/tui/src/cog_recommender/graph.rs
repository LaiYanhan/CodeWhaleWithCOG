...
impl TrajectoryGraph {
    pub fn observe_edge(
        ...
        entry.count += 1;
        entry.weight = (entry.weight + weight).min(1.0);
...
impl TrajectoryGraphUpdater {
    pub fn observe(
        ...
        for previous in &recent_snapshot {
            updated_edges.extend(self.observe_pair(graph, previous, &event));
        }
    ...
    fn observe_pair(
        ...
        if events_are_code_attention(previous, current) {
            updated_edges.push(graph.observe_edge(
                &previous_entity,
                &current_entity,
                EvidenceSource::CoAccess,
                0.05,
                "same session recently touched both entities",
            ));
            updated_edges.push(graph.observe_edge(
                &current_entity,
                &previous_entity,
        ...
        match (previous.kind, current.kind) {
            ...
            (TrajectoryKind::ErrorSignal, TrajectoryKind::EditEntity) => {
                updated_edges.push(graph.observe_edge(
                    &previous_entity,
                    &current_entity,
                    EvidenceSource::ErrorToEdit,
                    0.25,
                    "this error is often followed by editing the target",
                ));
            }
